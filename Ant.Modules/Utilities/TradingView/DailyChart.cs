...
public class DailyChart(string code) : Chart(code)
{
    public override IEnumerable<(bool isBullish, object atrStop)> InitializedAtrStopEnumerator(AtrStopResult[]? atrStop)
    {
    ...
    public override (object? macd, object? signal, object? histogram) InitializedMacdData(MacdResult[]? macd)
    {
        ...
        return (macdData, macdSignalData, macd?.Select((e, index) => new
        {
            time = (e.Date.Ticks - Cache.Epoch) / 10_000 / 1_000,
            value = e.Histogram,
    ...
    public override (object? linear, object? slope) InitializedSlopeData(IEnumerable<SlopeResult>? slope)
    {
        ...
        return (linearData, slope?.Select(e => new
        {
            time = (e.Date.Ticks - Cache.Epoch) / 10_000 / 1_000,
            value = e.Slope
        }));
    ...
    public override IEnumerable<(bool isBullish, object superTrend)> InitializedSuperTrendEnumerator(SuperTrendResult[]? superTrend)
    {
        Queue<object> data = new();
    ...
    public override (Series atrStop, Series superTrend, object indicator) UpdateFuturesIndicator(string code, Indicators? indicator)
    {
